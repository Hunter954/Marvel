...
export class Battle{
 ...
 startWave(){if(this.active||this.ended)return false;this.wave++;this.active=true;this.intermission=0;this.spawnTimer=.6;const count=6+this.wave*2+Math.min(20,this.stage*2);this.queue=[];for(let i=0;i<count;i++){const roll=this.rng();let type=this.stage>=2&&roll<.22?'brute':roll>.7?'runner':'drone';if(this.wave===this.totalWaves&&i===count-1)type='boss';this.queue.push(type);}this.emit('wave');return true;}
 ...
 fire(t,e){t.anim=t.animDuration=.42;t.action='attack';t.dir=e.x<t.x?-1:1;const h=t.hero;
 ...
 this.shots.push({x:t.x+t.dir*12,y:t.y-12,originX:t.x+t.dir*12,originY:t.y-12,age:0,target:e,tower:t,hero:h,damage:t.damage,dead:false});
 }
 impact(p){const e=p.target,t=p.tower,h=p.hero;if(h==='spider'){if(!e.dead&&e.web<=0){e.web=t.webDuration;e.webFactor=t.webFactor;this.effect('web',e.x,e.y,'#e1f7ff',.5);}return;}this.damage(e,p.damage,h==='hawkeye'||h==='strange');
 if(h==='iron'){this.effect('explosion',e.x,e.y,'#ffb44c',.55,{size:60});for(const a of this.targets(e,36))if(a!==e)this.damage(a,p.damage*.55);}
 if(h==='torch'){e.burn=4;e.burnDmg=p.damage*.48;this.effect('explosion',e.x,e.y,'#ff7a33',.5,{size:44});}
 if(h==='strange'){e.slow=2.4;this.effect('magic',e.x,e.y,'#60f6ba',.55,{size:46});}
 if(h==='thor'||h==='cap'){if(h==='cap')e.stun=.55;else this.effect('lightning',e.x,e.y,HEROES[h].color,.5,{size:58});const near=this.targets(e,70).filter(a=>a!==e).slice(0,h==='thor'?2:1);let prev=e;for(const a of near){this.effect('line',prev.x,prev.y,HEROES[h].color,.25,{x2:a.x,y2:a.y,width:3});this.damage(a,p.damage*.7);if(h==='cap')a.stun=.4;prev=a;}}
 }
 ...
 ultimate(t){if(this.ended||!this.towers.includes(t)||t.charge<100)return{error:'Ultimate ainda carregando'};let targets=this.targets(t,t.hero==='hulk'||t.hero==='torch'?120:t.range);if(t.hero==='spider')targets=targets.filter(e=>e.web>0);if(!targets.length)return{error:t.hero==='spider'?'Nenhum inimigo preso nas teias por perto':'Nenhum inimigo no alcance'};t.charge=0;t.anim=t.animDuration=.85;t.action='cast';this.emit('ultimate',{hero:t.hero});const h=t.hero;
 ...
 else {for(const e of targets.slice(0,h==='thor'?8:6)){this.damage(e,t.damage*3);if(h==='thor')this.effect('lightning',e.x,e.y,HEROES[h].color,.7,{size:78});this.effect('line',e.x-18,e.y-90,HEROES[h].color,.6,{x2:e.x,y2:e.y,width:5});}}
 return{ok:true};}
 ...
 update(dt){if(this.ended)return;this.time+=dt;if(!this.active){this.intermission=Math.max(0,this.intermission-dt);if(this.intermission<=0)this.startWave();}for(const fx of this.effects)fx.life-=dt;this.effects=this.effects.filter(x=>x.life>0);
 ...
 for(const p of this.shots){p.age+=dt;if(p.target.dead){p.dead=true;continue;}const d=distance(p,p.target),step=260*dt;if(d<=step){p.x=p.target.x;p.y=p.target.y;this.impact(p);p.dead=true;}else{p.x+=(p.target.x-p.x)/d*step;p.y+=(p.target.y-p.y)/d*step;}}
 this.enemies=this.enemies.filter(e=>!e.dead);this.shots=this.shots.filter(p=>!p.dead);
 if(this.active&&!this.queue.length&&!this.enemies.length){this.active=false;this.shots=[];if(this.wave>=this.totalWaves)this.finish(true);else{this.energy+=70;this.intermission=this.intermissionTotal=4;this.emit('waveEnd');}}
 }
}
