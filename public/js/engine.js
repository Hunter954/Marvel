...
export const HEROES={
 spider:{name:'Homem-Aranha',short:'ARANHA',role:'CONTROLE',color:'#f04d55',cost:100,unlock:0,range:108,rate:.8,damage:8,charge:22,skill:'Teia pegajosa',desc:'Teias reduzem a velocidade em 55% por 3 segundos.',ultimate:'Teia de recuo',ultDesc:'Puxa os inimigos presos no alcance 3 blocos para trás.'},
...
export const freshSave=()=>({version:2,stage:1,coins:0,diamonds:0,unlocked:{spider:true,iron:true},upgrades:{},wins:0,kills:0,best:1,sound:true});
export function normalizeSave(s){const d=freshSave();if(!s||typeof s!=='object')return d;for(const k of ['stage','coins','diamonds','wins','kills','best'])d[k]=Math.max(k==='stage'||k==='best'?1:0,Math.min(1e9,Math.floor(Number(s[k])||d[k])));for(const id of Object.keys(HEROES)){d.unlocked[id]=id==='spider'||id==='iron'||s.unlocked?.[id]===true;d.upgrades[id]=Math.min(10,Math.max(1,Math.floor(Number(s.upgrades?.[id])||1)));}d.sound=s.sound!==false;return d;}
export const upgradeCost=level=>100*level;
export function purchase(save,id){const h=HEROES[id];if(!h)return false;if(!save.unlocked[id]){if(save.diamonds<h.unlock)return false;save.diamonds-=h.unlock;save.unlocked[id]=true;save.upgrades[id]=1;}else{const lvl=save.upgrades[id]||1;if(lvl>=10||save.coins<upgradeCost(lvl))return false;save.coins-=upgradeCost(lvl);save.upgrades[id]=lvl+1;}return true;}
...
export class Battle{
 ...
 impact(p){const e=p.target,t=p.tower,h=p.hero;this.damage(e,p.damage,h==='hawkeye'||h==='strange');
 if(h==='spider'){e.slow=3;this.effect('web',e.x,e.y,'#e1f7ff',.35);}
 ...
 ultimate(t){if(this.ended||!this.towers.includes(t)||t.charge<100)return{error:'Ultimate ainda carregando'};let targets=this.targets(t,t.hero==='hulk'||t.hero==='torch'?120:t.range);if(t.hero==='spider')targets=targets.filter(e=>e.slow>0);if(!targets.length)return{error:t.hero==='spider'?'Nenhum inimigo preso nas teias por perto':'Nenhum inimigo no alcance'};t.charge=0;t.anim=.8;this.emit('ultimate',{hero:t.hero});const h=t.hero;
 if(h==='iron'||h==='cyclops'){this.beam(t,targets[0],h==='iron'?160:200,40,t.damage*5,HEROES[h].color);}
 else if(h==='spider'||h==='strange'||h==='cap'){for(const e of targets){e.d=Math.max(0,e.d-(h==='strange'?160:h==='spider'?120:80));e.slow=3;if(h==='strange')e.stun=2;if(h==='cap')this.damage(e,t.damage*3);const p=position(this.map,e.d);this.effect('line',e.x,e.y,HEROES[h].color,.6,{x2:p.x,y2:p.y,width:3});e.x=p.x;e.y=p.y;}this.effect('ring',t.x,t.y,HEROES[h].color,.6,{radius:t.range});}
 ...
 for(const e of this.enemies){if(e.dead)continue;e.slow=Math.max(0,e.slow-dt);e.stun=Math.max(0,e.stun-dt);if(e.burn>0){e.burn-=dt;this.damage(e,e.burnDmg*dt,true);}if(e.dead)continue;if(!e.stun)e.d+=e.speed*(e.slow>0?.45:1)*dt;const p=position(this.map,e.d);e.x=p.x;e.y=p.y;e.dir=p.dx<0?-1:1;if(e.d>=this.map.length){e.dead=true;this.lives-=e.type==='boss'?5:1;this.emit('leak');if(this.lives<=0){this.lives=0;this.finish(false);return;}}}
 for(const t of this.towers){t.anim=Math.max(0,t.anim-dt);if(!this.active)continue;t.charge=Math.min(100,t.charge+100/HEROES[t.hero].charge*dt);t.cool-=dt;if(t.cool<=0){const e=this.targets(t)[0];if(e){this.fire(t,e);t.cool=t.rate;}}}
