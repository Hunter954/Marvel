...
import {Battle,HEROES,generateMap,normalizeSave,freshSave,purchase,exchange,position,grantStarter,purchaseQuote} from '../public/js/engine.js';
...
test('mixed diamond and gold purchase charges exactly the shown quote and is atomic',()=>{
 const s=normalizeSave({coins:200,diamonds:2});const q=purchaseQuote(s,'thor');assert.equal(q.gold,200);assert.equal(q.diamonds,2);assert.equal(q.convert,2);assert.ok(q.affordable);assert.ok(purchase(s,'thor'));assert.equal(s.coins,0);assert.equal(s.diamonds,0);assert.ok(s.unlocked.thor);const before=JSON.stringify(s);assert.equal(purchase(s,'strange'),false);assert.equal(JSON.stringify(s),before);
});
test('Spider never damages or refreshes webbed enemies and two spiders reserve separate targets',()=>{
 const b=new Battle(normalizeSave(null),7);b.energy=1000;const p=b.map.pads.find(p=>b.map.cells.some(q=>Math.hypot(p.x-q.x,p.y-q.y)<60));const t=b.place('spider',p.c,p.r).tower;const p2=b.map.pads.find(q=>q!==p&&Math.hypot(q.x-p.x,q.y-p.y)<70);const t2=b.place('spider',p2.c,p2.r).tower;
 const d=b.map.cells.findIndex(q=>Math.hypot(p.x-q.x,p.y-q.y)<60)*40;b.active=true;
 for(let i=0;i<3;i++){b.spawn('boss');Object.assign(b.enemies.at(-1),{d:d+i*5,speed:0,hp:1000,maxHp:1000,...position(b.map,d+i*5)});}
 b.enemies[2].web=5;for(const t of b.towers)t.cool=0;b.update(.001);const shots=b.shots.filter(p=>p.hero==='spider');assert.equal(shots.length,2);assert.notEqual(shots[0].target,shots[1].target);assert.ok(shots.every(p=>p.target!==b.enemies[2]));
 for(let i=0;i<20;i++)b.update(.05);assert.ok(b.enemies.every(e=>e.hp===1000));assert.ok(b.enemies.every(e=>e.web>0));assert.equal(b.shots.length,0);
 const e=b.enemies[0],remaining=e.web;b.impact({target:e,tower:t,hero:'spider',damage:999});assert.equal(e.web,remaining);assert.equal(e.hp,1000);
 e.web=0;e.slow=3;t.cool=0;t2.cool=10;b.update(.001);assert.ok(b.shots.some(p=>p.target===e));
});
